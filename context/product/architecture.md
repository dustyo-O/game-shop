...
- **Hosted instance:** Neon, connected through its pooled (PgBouncer, transaction mode) endpoint, which speaks the plain Postgres wire protocol — so the same `pg` driver runs locally against the Compose container and in production against Neon, and only `DATABASE_URL` differs. Migrations and the seed go to Neon's direct endpoint from an operator's machine (`DATABASE_URL=<neon-direct> pnpm db:deploy`), never from a build. _Alternatives: Supabase or Vercel Postgres (both fine); Neon chosen for instant provisioning and a pooled endpoint that needs no special driver._ _Amended in Phase 6 (spec 006): was "…and a serverless driver that supports real transactions"; that driver was never adopted — see the query-layer bullet — and the pooled endpoint's plain wire protocol is the property that matters._
- **Local instance:** Postgres 16 in Docker Compose, so the same engine runs in development and production.
- **Query layer:** Drizzle ORM with `drizzle-orm/node-postgres` over `pg`, locally and live. _Amended in Phase 6 (spec 006): was `drizzle-orm/neon-serverless`; struck because one driver keeps two arguments valid in production rather than only on a laptop — §7's four-process `max: 1` measurement was taken with this pool, and the prepared-statement argument in `packages/db/src/client.ts` is about this driver's unnamed parse — and because a Vercel Node function can open the TCP socket the WebSocket driver exists to avoid. PgBouncer transaction mode was audited against the codebase (no session state: no `SET`, advisory locks, `LISTEN`, cursors or `.prepare(`); the grep is quoted in `client.ts`._ _Alternatives: Prisma — genuinely capable here (interactive transactions, atomic increments, and column-to-column comparison via field references all work natively); its one real gap is that it has no pessimistic locking in the query API, so `FOR UPDATE` and `FOR UPDATE SKIP LOCKED` drop to `$queryRaw`. It is also heavier in serverless, though driver adapters have narrowed that. TypeORM: closest to Nest conventions and has first-class locking, but weaker type inference. Drizzle chosen for typed locking and the lightest serverless footprint._
...
- **Hosting:** Vercel — `apps/web` as a static build, `apps/api` as a single Node serverless function that bootstraps Nest once and caches it across invocations. _Alternatives: Railway or Fly.io for a long-lived API process (simpler concurrency story, but a long-lived process would let an in-memory lock accidentally paper over a missing database guarantee — the serverless split proves it isn't there)._ _Amended in Phase 6 (spec 006): as deployed — the repository root is the project; `api/index.js` re-exports `apps/api/dist/vercel.js`, the one file Vercel builds as a function; `vercel.json` carries `framework: null`, `installCommand: pnpm install --frozen-lockfile`, `buildCommand: pnpm run build`, `outputDirectory: apps/web/dist`, `regions: ["fra1"]` (paired with the Neon project's `aws-eu-central-1`, so the one round trip a `max: 1` pool repeats on every statement stays inside one region), `functions."api/index.js".maxDuration: 60`, and three rewrites in order — `/api/(.*)` and `/internal/(.*)` to the function with the original `req.url`, everything not under `api/`, `internal/` or `assets/` to `index.html`. Fluid Compute is **off** — a project setting, on by default on a fresh project, turned off through the project API before the first deploy — so an instance serves one invocation at a time and concurrency is met with more instances; turning it off drops the project's default function timeout from 300 s to 10 s, which is why `maxDuration: 60` is a key and not a comfort. A misconfigured instance answers `503 { status: "misconfigured", error }` for its lifetime, and `GET /api/health` — `200` with `instance_id`, `runtime: "vercel"`, `supplier_timeout_ms` — is the boot probe. Measured on the first deploy: platform init 1.81 s plus Nest `init()` 29 ms on a cold instance, ~0.15 s warm; the function log's scheduler line reads `implementation: "wait_until"`. The live shop is `https://game-shop-ochre.vercel.app`; `docs/walkthrough/phase-6-slice-5-live.md`._
- **Why serverless strengthens the claim:** concurrent requests land in separate processes, so passing the fifty-webhook scenario against the deployed URL is direct evidence that correctness lives in Postgres and not in one process's memory. This is the README's strongest sentence. _Amended in Phase 6 (spec 006): measured on the alias — the fifty webhooks were answered by `30 distinct instance(s)`, the twenty promo applications by 18, and `pnpm race` passed `9/9` twice in a row; the recorded run is `docs/walkthrough/evidence/phase-6-live-race-run.txt`, and §7 carries the witness and its limits._
- **Local development:** Docker Compose for Postgres only; the API and web run from source (`pnpm dev`), the same way they run on Vercel — as a function and a static build. One-command startup (`pnpm dev:stack`), seeds included. The assignment accepts a fully local setup, so this is the guaranteed path; the deployment is the bonus. _Amended in Phase 6 (spec 006): was "Docker Compose (Postgres + API + web)"; the API and web were never containerised, and a container image would be a second deployment shape to keep honest beside the function._
...
- **Serverless timeouts — two hang scenarios, and they need opposite orderings.** The supplier stub's deliberate hang is configuration, not a constant, and which side of `SUPPLIER_TIMEOUT_MS` it lands on decides which scenario is being staged. Both are worth having as named checks; neither substitutes for the other.
  ...
  **The trap's ordering is the one the retry policy above turns on, and it was written backwards here until Phase 3.** The old text gave `hang < SUPPLIER_TIMEOUT_MS < ceiling` as the rule for every case, which produces no timeout at all: the client waits, the supplier answers, and a timeout check staged that way passes having exercised nothing. It was not simply inverted — it describes the first row, and was being cited as the basis for the second.
  ...
  `SUPPLIER_TIMEOUT_MS < ceiling` is what stops the second from pre-empting the first, and it holds in both rows: **a timeout must always be observed as a timeout, never as a killed function.** `hang_ms < ceiling` is the trap row's right-hand term, and it is what leaves the supplier time to finish and have a code on file for the re-probe to find. _Amended in Phase 6 (spec 006): the live numbers in the trap row are `5000 < 6500 < 60 000` — `SUPPLIER_TIMEOUT_MS=5000`, a hang the check derives from the target's own `GET /api/health` (`supplier timeout 5000 ms (from target /api/health)`), and `maxDuration: 60`; on the alias the order settled `delivered` about 6 s after the webhook, the re-probe having found the code on A's ledger._
...
- **Key pool:** the fifty supplied keys live in `supplier_keys` as supplier-side inventory. Exhausting it is how the empty-pool scenario is produced; restocking a live pool is adding rows. Two demo affordances on the supplier's side of the boundary stage the scenario without fifty purchases: `POST /internal/suppliers/keys/drain` claims every unclaimed key under a run-scoped sentinel (`drain_<token>_<id>`), and `POST /internal/suppliers/keys/restock` releases only sentinel claims — `LIKE 'drain\_%'`, the literal-underscore escape — so a key a shopper was shown is never resold. _Amended in Phase 6 (spec 006): was "restocking is an admin action"; the sentence was written before the affordances existed and did not say what an admin restock may and may not touch._
...
- **Concurrency proofs must run across separate API processes.** This is the single most important rule in this section, and it was learned by measurement rather than reasoning. `packages/db` sets the connection pool to `max: 1` per instance — the serverless shape — so within one process a transaction holds the only connection for the whole of `BEGIN … COMMIT` and a second concurrent claim queues **in Node, before a byte reaches Postgres**. `FOR UPDATE SKIP LOCKED` never skips, because nothing else holds a row lock when the subquery looks. The consequence: a claim written with **no locking at all** behaves identically to the correct one. Measured, with the key claim weakened to an unlocked `SELECT`-then-`UPDATE` and twenty distinct request ids:
  ...
  _Amended in Phase 6 (spec 006): the rule has a second measurement and a second witness. Locally the proof of "separate processes" is the database's — one backend pid per instance in `pg_stat_activity`. A reviewer pointed at the live shop holds no database, so every response also carries `x-instance-id`, one UUID minted per process (`apps/api/src/instance-identity.ts`); `race:harness` counts distinct values across eight concurrent `GET /api/health` (`PASS` at ≥ 2 against an external target, `FAIL` at exactly 1 naming Fluid Compute, `INFO` locally where an id per port is a tautology and the pids decide), and `webhooks`, `same-event` and `promo` print the count for their own burst. On the alias with Fluid Compute off, the live run is the second measurement of this rule: `30 distinct instance(s)` for the fifty webhooks, 18 for twenty promo applications, 9 for twenty redeliveries of one event, 5 across the harness's eight health calls — one key, three winners, `9/9` twice; the author's run with the direct host forwarded counted `26 distinct backend pid(s)` where the local harness counts 4. Each witness has a limit, printed with it: a distinct id proves a distinct process, not that the processes overlapped in time; a backend seen through a transaction-mode pooler is a pooler-owned server connection, not one per instance; and on this platform every instance's log line reads `pid: 4`, so the process id itself is no witness at all. The HTTP witness's weak spot is a cheap endpoint on a cold platform — one run's harness read K = 1 from eight ~1 ms health calls served back to back by a single warm instance while the work checks of the same run were answered by 17, 10, 8 and 19 — and the remedy is a re-run, never a retry: a witness that retries until it sees ≥ 2 cannot fail. `docs/walkthrough/phase-6-slice-5-live.md` §3–§5._
...
- **Test layout:** Vitest, configured in `apps/api/vitest.config.ts`, with tests under `apps/api/test/`:
  ...
  - Both assert the seeded baseline before and after, and clean up what they wrote, so they are re-runnable with no manual reset. Restoring `claimed_by_request_id = null` is a thing **no production path ever does** — a key is claimed by exactly one request forever, and restocking a live pool is adding rows. Outside those paths, two demo affordances clear a claim, each scoped so a delivered key can never be resold: the harness's cleanup (only the request ids the test derived) and the demo reset `POST /api/admin/demo/reset` (every key, only inside the transaction that deletes every `deliveries` row); `POST /internal/suppliers/keys/restock` releases only sentinel claims. _Amended in Phase 6 (spec 006 technical-considerations §2.4): was "a thing **only a test may do** — production has no un-claim, by design"; the demo reset and the drain/restock pair are not tests, so the sentence had to say what they are and how each is bounded._
...
- **Adversarial scripts taking a base URL** (`race:webhooks` for fifty webhooks on one order, `race:same-event`, `race:create-order`, `race:promo`, `recover:out-of-stock`, `recover:timeout`, `webhook:before-order`) arrive with the phases that own their scenarios. Written against a configurable base URL so the identical script runs locally and against the deployed system — which is the strongest form of the claim, since serverless instances share neither kernel nor clock. _Amended in Phase 6 (spec 006): they did — `RACE_BASE_URLS=https://game-shop-ochre.vercel.app ADMIN_TOKEN=<the demo token> RACE_DEMO_RESET=1 pnpm race` ran all nine against the deployed shop, `9/9` twice in a row, with the 39 database-side assertions reported as `SKIP` by name and the demo reset at the end; `scripts/race/README.md`, "External mode"._
...
- **Serverless costs cold starts.** The fifty-webhook scenario may run slower than it would against a long-lived process. That is a fair price for proving the guarantees are not process-local, and the scripts also run locally. _Amended in Phase 6 (spec 006): measured — 1.81 s of platform init plus 29 ms of Nest on a cold instance, 2.38 s end to end for a cold second invocation (the simulator's self-call), ~0.15 s warm; Neon suspends its compute after five idle minutes (observed at 5 min 15 s) and the first query then pays ~0.8 s, while the platform kept the instance warm past six minutes — two clocks, and a shopper after a pause pays whichever has expired. The live profile is sized to the measurement: `SUPPLIER_TIMEOUT_MS=5000` (the local 2000 would read a healthy cold supplier as `unknown`), `SUPPLIER_MAX_PROBES_PER_REQUEST=2` (`worst_case_ms 20000`), `maxDuration: 60` above both — necessary, since turning Fluid Compute off had dropped the project's default to 10 s. On the alias the fifty-webhook check took 1539 ms and the trap check ~6 s._
...
- **A test affordance on order creation.** The API accepts an explicit order id behind a configuration flag, used only by seeds and the "webhook before order" script. Without it that scenario cannot be staged deterministically, since order ids are otherwise server-generated. _Amended in Phase 6 (spec 006): the affordance is on in the live demo — `ALLOW_CLIENT_SUPPLIED_ORDER_ID=true` — so `pnpm race before-order` runs against the alias. The flag's "must never be set where real shoppers can reach it" stands; the demo is the one named exception, because it has no real shoppers and `pnpm demo:reset` returns it to baseline, and the boot-time `warn` says so on every instance._
- **A demo affordance on the whole shop.** `POST /api/admin/demo/reset`, behind the admin bearer token, returns the shop to its seeded state in one transaction — every order's row lock first (`lock_timeout` 5 s), then the six deletes (`orders`, `deliveries`, `issuance_attempts`, `promo_redemptions`, `payment_events`, `supplier_requests`), promo counters to zero, every supplier key released, every behaviour row back to baseline — and is idempotent: a second call returns zeros and `changed: false`. Beside it, on the supplier's side of the boundary, `POST /internal/suppliers/keys/drain` and `/restock` stage the empty pool under a run-scoped sentinel (§6). They exist because a reviewer's run against the deployed shop has no database to clean up with: one run leaves 38 orders, 55 payment events and 6 deliveries, and `RACE_DEMO_RESET=1` (the runner, after the last check) or `pnpm demo:reset` (the operator) takes them off so the next run starts at baseline — which is what makes "twice in a row" true. The admin token the live demo uses is published in the README on purpose: the shop has no real shoppers, the worst the token allows is what the reset undoes, and it differs from the local default so it can be labelled as the demo's. Local checks never call the reset; they clean up their own rows, and the runner refuses to call it in local mode. _Added in Phase 6 (spec 006), beside the two affordances above; technical-considerations §2.4, R9, R16._
