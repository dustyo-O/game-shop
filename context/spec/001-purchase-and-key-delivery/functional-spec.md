...
- **As a** shopper, **I want to** receive my key automatically once I have paid, **so that** I do not have to wait for anyone or ask for it.
  - **Acceptance Criteria:**
    ...
    - [x] Given the shopper stays on the order page after paying successfully, when the order moves from awaiting payment through being processed to delivered, then the page shows each change without the shopper reloading it.
...
- [2026-09-08] — Spec 002 Slice 4 — §2.4's second criterion is restored to its original wording: the page shows each change without the shopper reloading it. Confirmed live in nine browser purchases, every one of which showed an intermediate stage — «Оплачен, готовим ключ» or «Выдаём ключ» — held on screen for about a second, with no reload. The note qualifying the weakened wording is removed with it. What changed is worth stating precisely, because it is not what it appears to be: the stages did **not** become longer. Measured directly against the API, the run from the payment service being answered to the key being delivered still takes 25-65ms, much as it did before. What moved is *when the shop answers*. Previously the shop did the work first and answered afterwards, so a page that refreshed itself on hearing the answer was already too late and saw only the finished order. The shop now answers first and works afterwards, so the same refresh lands while the work is still in progress and catches a genuine intermediate stage — which then stays on screen until the next scheduled check a second later. The promise is therefore restored by the change of order, not by any slowing down. One consequence should be recorded honestly: because the visible stage depends on a refresh landing inside a window of a few tens of milliseconds, it is a well-timed observation rather than a guaranteed one. It was proven to be load-bearing by removing that refresh, at which point three of three purchases collapsed straight from awaiting payment to delivered, exactly as they did before this phase.
