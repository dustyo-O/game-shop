...
**One `document` click listener, no listener on the Каталог button.** It classifies `event.target` (guarded with `instanceof Element`): inside the button → `Toggle`; inside the overlay → `InsideClick`; otherwise → `OutsideClick`. Because the opening click and the outside check are one listener making one decision, the bubbling-order problem — the opening click also being seen as an outside click — cannot occur. Rejected: a button listener plus a document listener with `stopPropagation()` — stopping is untargeted, so it becomes the page's exemption mechanism and hides events from whoever is above: the buy feature's delegated listener for any click stopped inside the row, and the menu's own `document` listener, which needs the Купить click to reach it; registering the document listener on open via `setTimeout(0)` (a hack around ordering). `click`, not `pointerdown`: the spec says clicks, and `pointerdown` would close the menu when a shopper starts a drag-select. `document`, not the page root: on a wide screen the grey ground beside the column is outside `#app`'s children.
...
- Look: `cursor: pointer` and a hover state on all of them — the functional spec's scope line is "visibly present and quietly inert", and `cursor: default` or `disabled` would read as broken. `:active` feedback is fine: "promises no result" is about spinners, messages and navigation, not a button looking pressed.
...
| # | Risk | Mitigation |
| --- | --- | --- |
| R1 | Back/forward cache restores the page with the timer cancelled → banner dead after back (§2.7 crit 4) | `pageshow` + `persisted` → restart. **Measured:** removing Playwright's `--disable-back-forward-cache` does not restore eligibility — any attached CDP session disqualifies the page (`persisted: false`, `notRestoredReasons: [{ reason: "masked" }]`), so no automation can reach this path; it is a manual check in real Chrome, recorded in the walkthrough. |
...
| R20 | Chromium not installed on the reviewer's machine → the suite fails before the first test | `pnpm exec playwright install chromium` in the root README once Phase 6 writes it (there is none yet — the command lives in the config's preflight message, §4.2 and `architecture.md` §7 until then), and a config-level check that prints that command. |
...
| `entities/product/api/products-api.test.ts` | `parseCatalogResponse` (10 cases): `image: null` ok, string ok, `42` throws `CatalogResponseError`, a *missing* key throws (the API always emits it — `JSON.stringify` omits `undefined`, never `null`); `price_minor: null` throws; non-array body throws; row order preserved | Write-first: 5 pass / 5 fail before the reader exists — every failure an `image` case. Then accept any `image` (`row["image"] ?? null`) → the `42` and missing-key cases fail. |
...
| `e2e/banner.spec.ts` | §2.2 all seven (+ the arrow-hover case) | `page.clock.install({ time })` + `pauseAt` before `goto`; `aria-current` on dots, `hidden` on slides; the clock sequence above |
