...
> **Amended during slice 5 — the ladder could not serve a retry.** Branch 5 read "untried" as *a provider with no attempt row for this order*, so after `a/1 failed, b/2 failed` an operator retry fell straight through to `settleRefused` and **re-settled the order without asking anyone** — measured: `still_out_of_stock` returned with `request_id: …_b_2` and no new attempt row written. That fails §2.5's first criterion and contradicts §1.2's own last row (`Operator retry after both refused → a, 3, req_ord_x_a_3`).
>
> The fix is one branch at the bottom plus a new input the attempt rows genuinely cannot carry: **`IssuanceRound`** (`Continuing` by default, `Fresh` only on the *opening* turn of an operator entry, via a `satisfies Record<IssuanceEntry, IssuanceRound>` table). One retry buys one more round, not an unbounded number, because every recomputation inside a walk takes the default.
>
> **It cannot reorder the guard** — the `Fresh` branch sits below the outstanding check, so `probe` and `settleNeverEstablished` still outrank everything and the hard rule is untouched. The rejected alternative was synthesising the rung in the runner, which would be the second path into issuance the design exists to prevent.
>
> **Closed in slice 5's verification:** the alphabet was extended with the round axis — 1,788,098 histories, zero violations under either round, and every history where `Continuing` settles refused converts to `askFirst` under `Fresh` (6,272 of 6,272). A mutant with the `Fresh` branch above the guard produced 1,024 violations against 0 for the shipped ladder.
>
> **Original note, kept for the record:** slice 3's exhaustion over 894,049 histories predates this input. It still exhausts the *attempt-history* space but no longer spans the ladder's whole input space, so the hard rule is currently proven for `Continuing` and argued structurally for `Fresh`. Extending the alphabet with the round axis is slice 5's verification.
...
Control endpoint `PUT /internal/suppliers/:provider/behaviour` — on the **supplier side**, not under `/api`, because it is the reviewer's console into a simulated external service. Restocking was specified as `POST /internal/suppliers/keys`, which **inserts new rows** — **but no task built it, and it does not exist.** Found while writing slice 7's checks. `recover:out-of-stock` restocks over direct SQL instead, the technique `packages/db/src/schema/supplier.ts` explicitly reserves for tests, and every assertion §12 lists is exercised. Nothing in the functional spec requires a wire-level restock; this was an operator convenience the plan described and never scheduled. If it is ever built, `recover-out-of-stock.ts`'s header names the one place to point at it. Restocking, however it is done, must **insert** new rows and; it does not un-claim existing ones (§7 of the architecture: restoring `claimed_by_request_id = NULL` is a thing only a test may do).
...
> **Wire shape pinned during implementation.** §8 originally fixed the per-order field names but left two things open, and the operator's view was built before the endpoint existed — so it had to choose. Both are now fixed by the consumer, and the endpoint must match rather than the reverse:
>
> - the top-level array is **`orders`** (alongside `count`, `truncated`, `message`);
> - each entry of `attempts` carries **`provider`** (string), **`attempt`** (number), **`status`** (string), **`probe_count`** (number), **`last_error`** (string | null).
>
> `status` is a plain string on the wire, not a union, deliberately: `issuance_attempts.status` is `text` with no CHECK, and an unrecognised value must reach the screen as itself rather than be coerced into one the shop recognises. A mismatch surfaces as the view's `error` state naming the exact field, not a blank screen.
...
| **R2** | **The RED validation comes back green — again.** Phase 2 removed `FOR UPDATE` and every assertion held, because the deterministic id and I3/I5/I6 masked it. The same masking applies: break the ladder's `unknown` guard and `deliveries_order_id_key` still gives the shopper one key. | **Assert stock accounting, not the shopper's key count**: `count(*) FROM supplier_keys WHERE claimed_by_request_id IS NOT NULL` must equal `count(*) FROM deliveries`. That is §2.2's fifth criterion and the only assertion that fails. **It holds on settled outcomes only.** Measured on the probes-exhausted path: `claimed_keys 2, deliveries 1` — and that is *correct*, not a fault. A key genuinely was cut by the supplier and the shop never learned the code; that is the honest meaning of `unknown`, and it is exactly the loss this phase bounds rather than eliminates. Assert the equality on scenarios that reach `delivered`, `out_of_stock` or a definite `delivery_failed`. On a never-established outcome assert instead that **at most one** key is unaccounted for per outstanding attempt, and that the attempt row still reads `unknown` with `last_error` NULL. Weaken the **ladder rule**, not the lock. A count that varies between runs is the signature of a genuine race. |
