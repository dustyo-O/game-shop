...
This is also why the *look* is not neutralised. Every inert control has `cursor: pointer` and — from Slice 4 — a hover state, because the spec says they are "visibly controls" and a `disabled` attribute or a default cursor would read as broken. What §2.8 forbids is a *promise*: a spinner, a "coming soon", a message. A button that looks pressed and does nothing is honest; a button that shows a spinner and does nothing is not.
